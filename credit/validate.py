...
import requests
from .models import Application, Blacklist, Borrower, Program, StatusTypes
...
class IinGeneration:
    ...
    def date_of_birth(self):
        try:
            if datetime.strptime(str(self.iin)[:6], '%y%m%d').date():
                self.date_birth = datetime.strptime(str(self.iin)[:6], '%y%m%d').date()
    ...
    def twelve_num(self):
        try:
            iin = str(self.iin)
            tw_num = (1*iin[0] + 2*iin[1]  + 3*iin[2]  + 4*iin[3]  + 5*iin[4]  + 6*iin[5]  + 7*iin[6]  + 8*iin[7] + 9*iin[8]  + 10*iin[9] + 11*iin[10]   )
...
def checkapp(borrower, summa):
    ...
    if sum[0] != status and year[0] != status and ip[0] != status and blacklist[0] != status:
        status = StatusTypes.approved
    print(sum[1], year[1], ip[1], blacklist[1])
    if sum[1]: rejection_reason = sum[1]
