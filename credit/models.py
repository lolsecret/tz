...
class Program(models.Model):
    ...
    def __str__(self):
        return 'Минимальная и максимальная суммы: {0}/{1}. Минимальный и максимальный возраст: {2}/{3}'.format(self.min_sum, self.max_sum, self.min_year, self.max_year)
...
class Borrower(models.Model):
    ...
    def __str__(self):
        return 'ИИН заемщика: {0}'.format(self.iin)
...
class Application(models.Model):
    ...
    def __str__(self):
        return 'Сумма: {0}, Статус: {1}, Программа: {2}, Заемщик: {3}, Причина: {4}'.format(self.summa, self.status, self.program, self.borrower, self.rejection_reason)
